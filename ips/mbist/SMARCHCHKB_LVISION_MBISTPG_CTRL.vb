...
-          Version: 2017.1                                                       -
-       Created on: Fri Feb 22 16:55:43 CST 2019                                 -
----------------------------------------------------------------------------------
...
module SMARCHCHKB_LVISION_MBISTPG_CTRL (
    ...
    BIST_DATA_FROM_MEM1             ,          // Memory data for step 1
    ...
    BIST_COLLAR_EN1                                ,          // Enable for interface MEM1
    CHKBCI_PHASE                                   ,       
    ...
    input [31:0]     BIST_DATA_FROM_MEM1;
    ...
    reg              GO_EN;
    wire   [0:0]     STEP_COUNTER_INT;
    wire             STEP_COUNTER_FREEZE;
    wire             LAST_STEP;
    reg              NEXT_STEP_REG;
    wire             NEXT_STEP;
    wire             STEP_COUNTER_SI;
    wire             STEP_COUNTER_SO;
wire                 CTL_COMP_SI;
wire                 CTL_COMP_SO;
wire                 HOLD_EN;
...
//----------------------------------
    ...
    assign INIT_SIGNAL_GEN_REGS     = ~(SELECT_COMMON_OPSET_REG | STEP_COUNTER_FREEZE);
    assign INIT_DATA_GEN_REGS       = ~(SELECT_COMMON_DATA_PAT_REG | (STEP_COUNTER_FREEZE));
    ...
    assign BIST_COLLAR_EN1          = BIST_COLLAR_EN1_INT; // Memory ID: MEM1
    assign BIST_COLLAR_EN0_INT      = BIST_COLLAR_EN0_PRE;
    assign BIST_COLLAR_EN1_INT      = BIST_COLLAR_EN1_PRE;
    assign BIST_COLLAR_EN0_PRE      = BIST_ON & (STEP_COUNTER_INT == 1'b0) ;
    assign BIST_COLLAR_EN1_PRE      = BIST_ON & (STEP_COUNTER_INT == 1'b1) ;
    ...
    assign ALGO_DONE = LAST_TICK & LAST_STATE;
 
    assign NEXT_ALGO                = ALGO_DONE & ~LAST_STEP;
    //---------------
    //-- Next Step --
    //---------------
    always @ (posedge BIST_CLK_INT or negedge MBISTPG_ASYNC_RESETN) begin
        ...
        if (~BIST_HOLD_R) begin
            NEXT_STEP_REG <= ALGO_DONE & ~LAST_STEP;
        end
    end
...
assign CMP_EN = CMP_EN_INT;
    ...
    assign STEP_COUNTER_SI          = CTL_COMP_SO;
    assign MBISTPG_SO               = STEP_COUNTER_SO;
        
 
 
    wire BIST_COL_ADD_BIT0;
    assign BIST_COL_ADD_BIT0 = BIST_COL_ADD[0];
wire                 ENABLE_ERR_CNT_CAP;
assign ENABLE_ERR_CNT_CAP = STEP_COUNTER_FREEZE | (STEP_COUNTER_INT ==0);
    SMARCHCHKB_LVISION_MBISTPG_CTL_COMP MBISTPG_CTL_COMP (
       ...
       .BIST_DATA_FROM_MEM1         ( BIST_DATA_FROM_MEM1                       ), // i [31:0]
       .BIST_COLLAR_EN0             ( BIST_COLLAR_EN0_INT        ), // i
       .BIST_COLLAR_EN1             ( BIST_COLLAR_EN1_INT        ), // i
       .STEP_COUNTER                ( STEP_COUNTER_INT                          ), // i [0:0]
       ...
       .ENABLE_ERR_CNT_CAP          ( ENABLE_ERR_CNT_CAP                        ), // i
       .ESOE_RESET                  ( ESOE_RESET                                ), // o
       .TM                          ( LV_TM                                     )  // i
    );                       
 
    wire STEP_COUNTER_CLEAR_DEFAULT;
    assign STEP_COUNTER_CLEAR_DEFAULT = CLEAR_DEFAULT;
    SMARCHCHKB_LVISION_MBISTPG_STEP_COUNTER MBISTPG_STEP_COUNTER (
       .BIST_CLK                    (BIST_CLK_INT                               ), // i
       .BIST_EN                     (MBISTPG_EN_INT              ), // i
       .BIST_HOLD                   (BIST_HOLD_R_INT                            ), // i
       .BIST_ON                     (BIST_ON                                    ), // i
       .BIST_SHIFT_SHORT            (BIST_SHIFT_SHORT                           ), // i
       .SI                          (STEP_COUNTER_SI                            ), // i
       .SO                          (STEP_COUNTER_SO                            ), // o
       .LAST_TICK                   (LAST_TICK                                  ), // i
       .LAST_STATE                  (LAST_STATE                                 ), // i
       .BIST_ASYNC_RESETN           (MBISTPG_ASYNC_RESETN        ), // i
       .ESOE_RESET                  (ESOE_RESET                                 ), // i
       .PAUSETOEND_ALGO_MODE        (PAUSETOEND_ALGO_MODE                       ), // i
       .BIST_CLEAR_DEFAULT          (STEP_COUNTER_CLEAR_DEFAULT                 ), // i
       .NEXT_STEP                   (NEXT_STEP                                  ), // i
       .STEP_ALGO_SELECT            (STEP_ALGO_SELECT                           ), // o
       .STEP_COUNTER                (STEP_COUNTER_INT                           ), // o
       .STEP_COUNTER_FREEZE         (STEP_COUNTER_FREEZE                        ), // o
       .LAST_STEP                   (LAST_STEP                                  )  // o
    );
...
input                BIST_COLLAR_EN1;        
input [0:0]          STEP_COUNTER;        
...
reg  [31:0]          DATA_FROM_MEM_EXP_INT;
wire [31:0]          DATA_FROM_MEM_EXP;
reg  [31:0]          CMP_STAT_EN_INT;
wire [31:0]          CMP_STAT_EN;
wire [31:0]          EXPECT_DATA_REP_MEM0_INST0;
wire [31:0]          EXPECT_DATA_MEM0_INST0;
wire [31:0]          DATA_FROM_MEM0_EXP;
wire [31:0]          EXPECT_DATA_REP_MEM1_INST0;
wire [31:0]          EXPECT_DATA_MEM1_INST0;
wire [31:0]          DATA_FROM_MEM1_EXP;
wire                 COL_ADD_BIT0;
...
always @ (posedge BIST_CLK or negedge BIST_ASYNC_RESETN) begin
    ...
    if (ERROR_CNT_REG_RST) begin
        ...
        if (BIST_SHIFT_SHORT) begin
            ...
            if (~HOLD_EN) begin
                if (~CLEAR_DEFAULT) begin
                    if (RESET_REG_SETUP1 & STOP_ON_ERROR & FL_CNT_MODE0_SYNC & ENABLE_ERR_CNT_CAP) begin
...
wire FREEZE_GO_ID;
    ...
    //-------------------------------
    always @ (STEP_COUNTER or BIST_DATA_FROM_MEM0 or BIST_DATA_FROM_MEM1) begin
        case (STEP_COUNTER)
            1'b0 : DATA_FROM_MEM    = BIST_DATA_FROM_MEM0;
            1'b1 : DATA_FROM_MEM    = BIST_DATA_FROM_MEM1;
            default : DATA_FROM_MEM = 32'd0;
        endcase
    end
    ...
    assign EXPECT_DATA_REP_MEM1_INST0              = { // MemoryInstance: MEM1
                                        EXPECT_DATA,
                                        EXPECT_DATA,
                                        EXPECT_DATA,
                                        EXPECT_DATA,
    ...
    assign EXPECT_DATA_MEM1_INST0   = ~(CHKBCI_PHASE) ? EXPECT_DATA_REP_MEM1_INST0[31:0] : ({32{EXPECT_DATA_REP_MEM1_INST0[0]}} ^ {32{COL_ADD_BIT0}});
    ...
    assign DATA_FROM_MEM1_EXP       = {
                                        EXPECT_DATA_MEM1_INST0[31], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[30], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[29], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[28], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[27], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[26], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[25], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[24], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[23], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[22], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[21], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[20], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[19], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[18], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[17], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[16], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[15], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[14], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[13], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[12], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[11], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[10], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[9], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[8], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[7], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[6], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[5], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[4], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[3], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[2], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[1], // MemoryInstance: MEM1
                                        EXPECT_DATA_MEM1_INST0[0]  // MemoryInstance: MEM1
                                      };
 
    //-----------------
    //-- Expect Data --
    //-----------------
    always @ (STEP_COUNTER or DATA_FROM_MEM0_EXP or DATA_FROM_MEM1_EXP) begin
        case (STEP_COUNTER)
            1'b0 : DATA_FROM_MEM_EXP_INT           = {
            ...
            1'b1 : DATA_FROM_MEM_EXP_INT           = {
                                                       DATA_FROM_MEM1_EXP[31], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[30], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[29], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[28], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[27], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[26], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[25], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[24], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[23], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[22], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[21], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[20], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[19], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[18], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[17], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[16], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[15], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[14], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[13], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[12], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[11], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[10], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[9], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[8], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[7], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[6], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[5], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[4], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[3], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[2], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[1], // MemoryInstance: MEM1
                                                       DATA_FROM_MEM1_EXP[0]  // MemoryInstance: MEM1
                                                     };
            default : DATA_FROM_MEM_EXP_INT = 32'd0;
        endcase
    end
    ...
    //-- Comparator Enable --
    //-----------------------
    always @ (STEP_COUNTER or CMP_ID_SEL_R or BIST_COLLAR_EN0 or BIST_COLLAR_EN1) begin
        case (STEP_COUNTER)
            1'b0 : CMP_STAT_EN_INT                 = {
            ...
            1'b1 : CMP_STAT_EN_INT                 = {
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 32), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 31), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 30), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 29), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 28), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 27), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 26), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 25), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 24), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 23), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 22), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 21), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 20), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 19), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 18), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 17), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 16), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 15), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 14), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 13), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 12), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 11), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 10), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 9), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 8), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 7), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 6), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 5), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 4), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 3), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 2), // MemoryInstance: MEM1
                                        BIST_COLLAR_EN1 & (CMP_ID_SEL_R == 0 | CMP_ID_SEL_R == 1)  // MemoryInstance: MEM1
                                      };
            default : CMP_STAT_EN_INT = 32'd0;
        endcase
    end
...
module SMARCHCHKB_LVISION_MBISTPG_ADD_GEN (
    ...
    reg     [9:0]    AX_ADD_WIRE, BX_ADD_WIRE;
    reg     [9:0]    AX_ADD_REG;
    reg     [9:0]    BX_ADD_REG;
    reg     [2:0]    AY_ADD_WIRE, BY_ADD_WIRE;
    reg     [2:0]    AY_ADD_REG;
    reg     [2:0]    BY_ADD_REG;
    wire    [9:0]    AX_MASK;   
    wire    [9:0]    BX_MASK;
    ...
    reg    [9:0]     X_ADD_REG_MIN_DEFAULT;
    reg    [9:0]     X_ADD_REG_MAX_DEFAULT;
...
//----------------------------------------------
    ...
    always @(STEP_COUNTER) begin
       case (STEP_COUNTER)
          1'b0:      X_ADD_REG_MIN_DEFAULT         = 10'b0000000000;
          1'b1:      X_ADD_REG_MIN_DEFAULT         = 10'b0000000000;
       endcase  
    end
    ...
    // Algorithm: SMARCHCHKB Type: IC
    always @(STEP_COUNTER or MBISTPG_REDUCED_ADDR_CNT_EN) begin
       case (STEP_COUNTER)
          1'b0:      X_ADD_REG_MAX_DEFAULT         = MBISTPG_REDUCED_ADDR_CNT_EN ? 10'b0111111111 : 10'b0111111111;
          1'b1:      X_ADD_REG_MAX_DEFAULT         = MBISTPG_REDUCED_ADDR_CNT_EN ? 10'b1111111111 : 10'b1111111111;
       endcase
    end
...
//----------------------------------------------
    ...
    always @(STEP_COUNTER or MBISTPG_REDUCED_ADDR_CNT_EN) begin
       case (STEP_COUNTER)
          1'b0:      Y_ADD_REG_MAX_DEFAULT         = MBISTPG_REDUCED_ADDR_CNT_EN ? 3'b111 : 3'b111;
          1'b1:      Y_ADD_REG_MAX_DEFAULT         = MBISTPG_REDUCED_ADDR_CNT_EN ? 3'b111 : 3'b111;
       endcase
    end
...
//------------------------------------
    always @(STEP_COUNTER) begin
        case (STEP_COUNTER)
        1'b0:        AX_ADD_WIRE    = 10'b0000000000; // Algorithm: SMARCHCHKB
        1'b1:        AX_ADD_WIRE    = 10'b0000000000; // Algorithm: SMARCHCHKB
        endcase  
    end
...
//------------------------------------
    always @(STEP_COUNTER) begin
        case (STEP_COUNTER)
        1'b0:        AY_ADD_WIRE    = 3'b000; // Algorithm: SMARCHCHKB
        1'b1:        AY_ADD_WIRE    = 3'b000; // Algorithm: SMARCHCHKB
        endcase  
    end 
...
//------------------------------------
    always @(STEP_COUNTER) begin
        case (STEP_COUNTER)
        1'b0:        BX_ADD_WIRE    = 10'b0000000000; // Algorithm: SMARCHCHKB
        1'b1:        BX_ADD_WIRE    = 10'b0000000000; // Algorithm: SMARCHCHKB
        endcase  
    end 
...
//------------------------------------
    always @(STEP_COUNTER) begin
        case (STEP_COUNTER)
        1'b0:        BY_ADD_WIRE    = 3'b000; // Algorithm: SMARCHCHKB
        1'b1:        BY_ADD_WIRE    = 3'b000; // Algorithm: SMARCHCHKB
        endcase  
    end 
...
module SMARCHCHKB_LVISION_MBISTPG_SIGNAL_GEN (
    ...
    always @ (STEP_COUNTER) begin 
       case (STEP_COUNTER) 
          1'b0: OPSET_SELECT_WIRE = 3'b000; // OperationSet: SYNC
          1'b1: OPSET_SELECT_WIRE = 3'b000; // OperationSet: SYNC
       endcase 
    end     
...
//----------------------------------
    ...
    always @ (posedge BIST_CLK or negedge BIST_ASYNC_RESETN) begin
        if (~BIST_ASYNC_RESETN)
            MEMORY_CONTENT<= 2'b00;
        else
        if (RESET_REG_DEFAULT_MODE)
            case (STEP_COUNTER) 
            1'b0:    MEMORY_CONTENT <= 2'b00;
            1'b1:    MEMORY_CONTENT <= 2'b00;
            endcase 
        else
        if (BIST_WRITEENABLE & WDATA_CMD_SELECTED == 4'b1010)
            case (STEP_COUNTER) 
            1'b0:    MEMORY_CONTENT <= 2'b00; // BitSliceWidth: 1
            1'b1:    MEMORY_CONTENT <= 2'b00; // BitSliceWidth: 1
            endcase 
    end  
...
module SMARCHCHKB_LVISION_MBISTPG_STEP_COUNTER (
       BIST_CLK, 
       BIST_EN,
       BIST_HOLD,
       BIST_ON,
    ...
    output [0:0]     STEP_COUNTER;
    output           STEP_COUNTER_FREEZE;
    output           LAST_STEP;
    output [0:0]     STEP_ALGO_SELECT;
    reg  [0:0]       STEP_ALGO_SELECT;
    reg  [0:0]       STEP_COUNTER;
    reg              LAST_TICK_R;
    wire             NEXT_STEP;
    wire             STEP_COUNTER_RST;
    reg              STEP_COUNTER_FREEZE;
    wire             STEP_COUNTER_FREEZE_RST;

    always @ (STEP_COUNTER) begin
        case (STEP_COUNTER)
            1'b0 :   STEP_ALGO_SELECT = 1'b0; // SMARCHCHKB 
            1'b1 :   STEP_ALGO_SELECT = 1'b0; // SMARCHCHKB 
            default : STEP_ALGO_SELECT = 1'b0; 
        endcase 
    end

    assign LAST_STEP = STEP_COUNTER_FREEZE | (STEP_COUNTER == 1'b1);

    assign STEP_COUNTER_FREEZE_RST = ~BIST_HOLD & ~BIST_SHIFT_SHORT & BIST_CLEAR_DEFAULT;

    // synopsys sync_set_reset "STEP_COUNTER_FREEZE_RST"
    // synopsys async_set_reset "BIST_ASYNC_RESETN"
    always @ (posedge BIST_CLK or negedge BIST_ASYNC_RESETN) begin
        if (~BIST_ASYNC_RESETN)
            STEP_COUNTER_FREEZE <= 1'b0;
        else
        if (STEP_COUNTER_FREEZE_RST) begin
            STEP_COUNTER_FREEZE <= 1'b0;
        end else begin
            if (BIST_SHIFT_SHORT) begin
                STEP_COUNTER_FREEZE <= SI;
            end
        end
    end

    assign STEP_COUNTER_RST = ~BIST_HOLD & ~BIST_SHIFT_SHORT & BIST_CLEAR_DEFAULT | ESOE_RESET;
    
    // synopsys sync_set_reset "STEP_COUNTER_RST"
    // synopsys async_set_reset "BIST_ASYNC_RESETN"
    always @ (posedge BIST_CLK or negedge BIST_ASYNC_RESETN) begin
        if (~BIST_ASYNC_RESETN)
            STEP_COUNTER <= 1'b0;
        else
        if (STEP_COUNTER_RST) begin
            STEP_COUNTER <= 1'b0;
        end else begin
            if (BIST_SHIFT_SHORT) begin
                STEP_COUNTER <= STEP_COUNTER_FREEZE;
            end else begin
                if (~BIST_HOLD & NEXT_STEP) begin
                    STEP_COUNTER <= STEP_COUNTER_INC(STEP_COUNTER);
                end
            end
        end
    end

    assign SO = STEP_COUNTER[0];

    function automatic [0:0] STEP_COUNTER_INC;
        input [0:0] STEP_COUNTER;
        reg TOGGLE;
        integer i;
        begin
            STEP_COUNTER_INC[0] = ~STEP_COUNTER[0];
            TOGGLE = 1;
            for (i=1;i<=0;i=i+1) begin
                TOGGLE = STEP_COUNTER[i-1] & TOGGLE;
                STEP_COUNTER_INC[i] = STEP_COUNTER[i] ^ TOGGLE;
            end
        end
    endfunction

endmodule // SMARCHCHKB_LVISION_MBISTPG_STEP_COUNTER
