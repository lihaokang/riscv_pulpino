...
int rtc_init(void)
{
	...
	rtc->ctrl.ctrl_div = (uint32_t)0xffff;
	while (--delay);
...
int rtc_set_timev(uint32_t timev[])
{
	...
	if ((timev[0] < 2000) || (timev[0] > 2100)) return -1;
	if ((timev[1] == 0) || (timev[1] > 12)) return -1;
	if ((timev[2] == 0) || (timev[2] > 31)) return -1;
	if (timev[3] > 23) return -1;
	if (timev[4] > 59) return -1;
	if (timev[5] > 59) return -1;

	date.date_d = timev[2] % 10;
	date.date_td = timev[2] / 10;
	date.date_m = timev[1] % 10;
	date.date_tm = timev[1] / 10;
	date.date_y = (timev[0] % 100) % 10;
	date.date_ty = (timev[0] % 100) / 10;
	date.date_c = (timev[0] / 100) % 10;
	date.date_tc = (timev[0] / 100) / 10;

	time.time_s = timev[5] % 10;
	time.time_ts = timev[5] / 10;
	time.time_m = timev[4] % 10;
	time.time_tm = timev[4] / 10;
	time.time_h = timev[3] % 10;
	time.time_th = timev[3] / 10;
	time.time_dow = rtc_get_wday(timev[0], timev[1], timev[2]) + 1;
