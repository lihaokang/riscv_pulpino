...
-          Version: 2017.1                                                       -
-       Created on: Fri Feb 22 16:55:44 CST 2019                                 -
----------------------------------------------------------------------------------
...
/*------------------------------------------------------------------------------
     Module      :  SMARCHCHKB_LVISION_MEM1_INTERFACE
 
     Description :  This module contains the interface logic for the memory
                    module SHAA110_8192X8X4CM8
  ...
  always @ (posedge BIST_CLK_INT or negedge BIST_ASYNC_RESETN) begin
    if (~BIST_ASYNC_RESETN)
      ...
      SCAN_OBS_FLOPS    <= {
                          ...
                          WEB1_NOT_GATED       ^ WEB0_NOT_GATED       ^ A12                  ^ A11                  ,
                          A10                  ^ A9                   ^ A8                   ^ A7                   ,
                          A6                   ^ A5                   ^ A4                   ^ A3                   ,
                          A2                   ^ A1                   ^ A0                   
                           };
...
//--------------------------
   ...
   // Port: A12 LogicalPort: A Type: READWRITE {{{

   // Intercept functional signal with test mux
   always @( A12_IN or A12_TEST_IN or BIST_INPUT_SELECT_INT) begin
      case (BIST_INPUT_SELECT_INT) // synopsys infer_mux
      1'b0 : A12 = A12_IN;
      1'b1 : A12 = A12_TEST_IN;
      endcase
   end
   // Address logic during memory test
   wire   [2:0]                     BIST_COL_ADD_SHADOW_A;
   wire   [9:0]                     BIST_ROW_ADD_SHADOW_A;
   assign BIST_ROW_ADD_SHADOW_A[9] = BIST_ROW_ADD[9];
   assign #(10.0) A12_TEST_IN       = BIST_ROW_ADD_SHADOW_A[9];
...
endmodule // SMARCHCHKB_LVISION_MEM1_INTERFACE
