...
module acdc_check_pf64ak32ei40 (prog, mase, sera, cs, ae, oe, nvstr, ifren, tme, tmr, fastp, busy, 
                       prog_latch, sera_latch, mase_latch, 
                       addr, din, resetb, wrong_timing);

   input prog, mase, sera, cs, oe, ae, nvstr, ifren, tme, tmr, fastp, busy;
...
module pf64ak32ei40_core(cs, oe, ae, porst, ifren, nvstr, prog, sera, mase, tbit, tmr, tme, byte_1, oscen, osc, fastp, busy,  
				 prog_latch, sera_latch, mase_latch, outen, addr, addr_1, din, dout, wrong_timing, gangpgm, sg_stress, bl_stress, extpe, quad_pgm);
...
//address and data input behavioral
   ...
   always @(posedge ae or wrong_timing) 
   begin
...
//data output behavioral   
  ...
  always @(oe_o or posedge oe_outen)
  begin
     if(oe_outen == 1'b1)
        oe_lead_x = 1'b0;
     else if((oe_o == 1'b1) && (oe_outen == 1'b0))
        oe_lead_x = 1'b1;
  end
...
//                Read MEMORY
//-------------------------------------------------------------------
   always @(oe_outen or ae_outen or oe_lead_x) 
   if ($time > `InitTime)
   begin
      ...
      begin
         if((oe_o == 1'b0) && (oe_outen == 1'b0))
            dout = ALL00256[(`BIT_DATA-1):0];
         else if(((prog_latch == 1'b1) || (sera_latch == 1'b1) || (mase_latch == 1'b1)) && (oe_o == 1'b1))
            dout = ALLXX256[(`BIT_DATA-1):0];
         else
         begin
//-- when OE, AE setup, Dout output 'x'
            if((oe_lead_x == 1'b1) || (ae_outen == 1'b0))
//               dout = ALLXX256[(`BIT_DATA-1):0];
               dout = 32'habcd;
...
//Byte Mode
               ...
               begin
                  ...
                  if(read_addr_1 == 1'b0)
                     dout = {ALL00256[(`BYTEWIDTH-1):0],data_rd_temp[(`BYTEWIDTH-1):0]};
                  else
                     dout = {ALL00256[(`BYTEWIDTH-1):0],data_rd_temp[(`WORDWIDTH-1):`BYTEWIDTH]};
               end
//Byte Mode
               else if(byte_1 == 1'b0)
               begin
